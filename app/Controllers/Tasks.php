...
class Tasks extends Security_Controller
{
    ...
    private function _get_task_related_dropdowns($context = "", $context_id = 0, $return_empty_context = false)
    {
        ...
        if ($context === "client" && !$return_empty_context) {
            ...
            foreach ($clients as $client) {
                ...
                $clients_dropdown[] = array("id" => $client->id, "text" => $full_name);
            }
        }
    ...
    function save()
    {
        ...
        $status_id = $this->request->getPost('status_id') ? $this->request->getPost('status_id') : 1;
        ...
        $data = array(
            ...
            "points" => $this->request->getPost('points') ? $this->request->getPost('points') : 1,
            "status_id" => $status_id,
        ...
        if ($save_id) {
            ...
            $task_link = anchor(get_uri("tasks/view/" . $save_id), "Task #" . $save_id);
            $timeline_data = array(
                'client_id' => $data["client_id"],
                'title' => get_login_team_member_profile_link(),
                'caption' => timeline_label('created') . $task_link
            );
        ...
        if (is_date_exists($data->start_date)) {
            if ($show_time_with_task) {
                if (date("H:i:s", strtotime($data->start_date)) == "00:00:00") {
                    $start_date = "Starts: " . format_to_date($data->start_date, false);
                } else {
                    $start_date = "Starts: " .  format_to_relative_time($data->start_date, false, false, true);
                }
            } else {
                $start_date = "Starts: " . format_to_date($data->start_date, false);
            }
    ...
    function all_tasks($tab = "", $status_id = 0, $priority_id = 0, $type = "")
    {
        ...
        $collaborators_for_filters_dropdown = array(array("id" => "", "text" => "- " . app_lang("collaborators") . " -"));
        foreach ($assigned_to_list as $key => $value) {
    ...
    function all_tasks_list_data($is_widget = 0)
    {
        ...
        if ($is_widget) {
            ...
            if ($todo_status_id) {
                ...
                $options["collaborators"] = $this->login_user->id;
            }
        }
