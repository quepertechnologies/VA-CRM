...
class Partners extends Security_Controller
{
    ...
    function save()
    {
        ...
        $data = array(
            ...
            // "sec_applicant" => $this->request->getPost('sec_applicant'),
            // "sec_applicant_first_name" => $this->request->getPost('sec_applicant_first_name'),
            // "sec_applicant_last_name" => $this->request->getPost('sec_applicant_last_name'),
            // "sec_applicant_dob" => $this->request->getPost('sec_applicant_dob'),
            // "sec_applicant_phone" => $this->request->getPost('sec_applicant_phone'),
            // "sec_applicant_email" => $this->request->getPost('sec_applicant_email'),
            // "application_ids" => $this->request->getPost('application_ids'),
    ...
    function list_data()
    {
        ...
        $result["location_ids"] = get_ltm_opl_id(false, ',');
    ...
    private function _make_row($data, $custom_fields)
    {
        ...
        $row_data = array(
            $data->id,
            get_client_contact_profile_link($data->id, $data->first_name . " " . $data->last_name, array(), array('caption' => $data->unique_id, 'account_type' => $data->account_type, 'partner_type' => $data->partner_type)),
            $created_at,
            $data->com_percentage . '%',
            $data->phone,
            $data->email,
    ...
    function view($client_id = 0, $tab = "")
    {
        ...
        if ($client_id) {
            ...
            if ($client_info && !$client_info->is_lead) {
                ...
                $view_data['com_percentage'] = $client_info->com_percentage; // for account_type = Student
    ...
    function _make_checklist_row($data, $client_id = 0)
    {
        ...
        $options = modal_anchor(get_uri("partners/file_modal_form"), "<i data-feather='plus-circle' class='icon-16'></i> ", array("class" => "btn btn-default btn-sm", "title" => app_lang('add_file') . ": " . $data->label, "data-post-client_id" => $client_id, "data-post-doc_check_list_id" => $data->id, "data-post-doc_check_list_item" => $data->label));
    ...
    function checklist_modal_form()
    {
        ...
        return $this->template->view('partners/files/checklist_modal_form', $view_data);
    }
    ...
    function activity($client_id = 0)
    {
        ...
        return $this->template->view("partners/activity/index", $view_data);
    }
    ...
    function emails($client_id = 0)
    {
        ...
        return $this->template->view("partners/emails/index", $view_data);
    }
    ...
    function whatsapp($client_id = 0)
    {
        ...
        return $this->template->view("partners/whatsapp/index", $view_data);
    }
    ...
    private function _make_contact_row($data, $custom_fields, $hide_primary_contact_label = false)
    {
        ...
        $row_data = array(
            $user_avatar,
            $contact_link,
            anchor(get_uri("partners/view/" . $data->client_id), $full_name),
            $data->job_title,
            $data->email,
