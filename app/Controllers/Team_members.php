...
class Team_members extends Security_Controller
{
    ...
    function modal_form()
    {
        ...
        $view_data['location_dropdown'] = $this->make_locations_dropdown('- All Branches -');
    ...
    function add_team_member()
    {
        ...
        if ($user_id) {
            ...
            $this->Office_model->ci_save($office_data);
            ...
            if (!is_server_localhost() && $this->request->getPost('email_login_details')) {
    ...
    function send_team_login_emails()
    {
        ...
        foreach ($users as $user_data) {
            ...
            if (!is_server_localhost()) {
                ...
                $data = array('password' => password_hash($pwd, PASSWORD_DEFAULT));
                ...
                $parser_data["DASHBOARD_URL"] = base_url();
                $parser_data["LOGO_URL"] = "<img src='" . get_logo_url() . "'>";
                $parser_data["RECIPIENTS_EMAIL_ADDRESS"] = $user_data->email;

                $message = $this->parser->setData($parser_data)->renderString($email_template->message);
                $subject = $this->parser->setData($parser_data)->renderString($email_template->subject);

                $this->Users_model->ci_save($data, $user_data->id);
    ...
    private function _make_row($data, $custom_fields)
    {
        ...
        $user_role = $this->get_role_label($data->role_id, $data->is_admin);
        $full_name = $data->first_name . " " . $data->last_name . " <br><small>" . $user_role . "</small>";

        //check contact info view permissions
        $show_cotact_info = $this->can_view_team_members_contact_info();

        $location_label = "-";
        $location_id = get_tm_ol_id($data->id, true);
        if ($location_id) {
    ...
    function view($id = 0, $tab = "")
    {
        if ((int)$id * 1) {
            ...
            if ($user_info) {
                ...
                $view_data["show_offices"] = false;
                if ($this->login_user->is_admin) {
                    $view_data["show_offices"] = true;
                }
    ...
    function save_default_location($location_id = 0)
    {
        $location_id = clean_data($location_id);
        $data["location_id"] = $location_id ? $location_id : 0;

        if ($this->Users_model->ci_save($data, $this->login_user->id)) {
            echo json_encode(array("success" => true, 'message' => app_lang('record_updated')));
        } else {
            echo json_encode(array("success" => false, 'message' => app_lang('error_occurred')));
        }
    ...
    function save_account_settings($user_id)
    {
        ...
        if ($location_id) {
            $account_data["location_id"] = $location_id;
        }
    ...
    function offices($user_id)
    {
        ...
        if ($user_id) {
            $view_data['user_id'] = clean_data($user_id);
            return $this->template->view("team_members/offices/index", $view_data);
        }
        show_404();
    }

    function office_modal_form()
    {
        $location_dropdown = $this->get_locations_dropdown_for_filter("Office Location");
        $view_data['location_dropdown'] = $location_dropdown;
        $view_data['user_id'] = $this->request->getPost("user_id");

        return $this->template->view('team_members/offices/modal_form', $view_data);
    }

    function office_list_data($user_id)
    {
        $options = array('user_id' => $user_id);

        $list_data = $this->Office_model->get_details($options)->getResult();

        $result = array();
        foreach ($list_data as $data) {
            $result[] = $this->_make_office_row($data, count($list_data));
        }
        echo json_encode(array("data" => $result));
    }
    ...
    function save_office()
    {
        ...
        $save_id = $this->Office_model->ci_save($office_data);
        if ($save_id) {
            echo json_encode(array("success" => true, 'message' => app_lang('record_updated'), "data" => $this->_row_office_data($save_id), 'id' => $save_id));
        } else {
            echo json_encode(array("success" => false, 'message' => app_lang('error_occurred')));
        }
    }

    function _row_office_data($id)
    {
        $data = $this->Office_model->get_one($id);

        $list_options = array('user_id' => $data->user_id);
        $list_data = $this->Office_model->get_details($list_options)->getResult();

        return $this->_make_office_row($data, count($list_data));
    }

    function _make_office_row($data, $total_data)
    {
        $location_label = $this->get_location_label($data->location_id);
        if ($data->is_primary) {
            $location_label .= " <span class='mt0 badge' style='background-color: #27AE60;' title='Primary'>Primary</span> ";
        }
        $row_data = array(
            $data->id,
            $location_label,
        ...
        $delete_link = !$data->is_primary ? js_anchor("<i data-feather='star' class='icon-16'></i>", array('title' => "Mark as primary location", "class" => "edit", "data-id" => $data->id, "data-action-url" => get_uri("team_members/mark_as_primary_office"), "data-action" => "delete", "data-reload-on-success" => true)) : "";
        if ($this->_can_delete_team_member($data) && $total_data > 1) {
            $delete_link .= js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_office'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("team_members/delete_office"), "data-action" => "delete-confirmation"));
        }
    ...
    function mark_as_primary_office()
    {
        $this->validate_submitted_data(array(
            "id" => "required|numeric"
        ));
        ...
        $data_info = $this->Office_model->get_one($id);

        $options = array(
            'user_id' => $data_info->user_id,
            'is_primary' => 1
        );
        $data_list = $this->Office_model->get_details($options)->getResult();

        foreach ($data_list as $office) {
            $_data = array(
                'is_primary' => 0
            );
            $this->Office_model->ci_save($_data, $office->id);
        }

        $data = array(
            'is_primary' => 1
        );
        $save_id = $this->Office_model->ci_save($data, $id);
    ...
    function delete_office()
    {
        ...
        if ($this->Office_model->delete($id)) {
            echo json_encode(array("success" => true, 'message' => app_lang('record_deleted')));
        } else {
            echo json_encode(array("success" => false, 'message' => app_lang('record_cannot_be_deleted')));
        }
