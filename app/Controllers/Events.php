...
class Events extends Security_Controller
{
    ...
    function modal_form()
    {
        ...
        $view_data['project_id'] = $this->request->getPost('project_id') ? $this->request->getPost('project_id') : 0;
        $view_data['milestone_id'] = $this->request->getPost('milestone_id') ? $this->request->getPost('milestone_id') : 0;
    ...
    function save()
    {
        ...
        if ($save_id) {
            ...
            if ($data["client_id"]) {
                $_link = modal_anchor(get_uri("events/view/"), "Event #" . $save_id, array("data-post-id" => encode_id($save_id, "event_id"), "data-post-cycle" => 0, "title" => app_lang("event_details")));
                $timeline_data = array(
