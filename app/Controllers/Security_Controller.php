...
class Security_Controller extends App_Controller
{
    ...
    public $login_user;
    public $locations_dropdown = array();
    ...
    public function __construct($redirect = true)
    {
        ...
        $this->locations_dropdown = $this->make_locations_dropdown();
    }

    // get current milestone for project
    protected function get_current_milestone_id($project_id = 0, $return_all = false)
    {
        $options = array('project_id' => $project_id, 'is_current' => 1);
        $current_milestone = $this->Milestones_model->get_one_where($options);
        if ($current_milestone) {
            if ($return_all) {
                return $current_milestone;
            }
            return $current_milestone->id;
        } else {
            if ($return_all) {
                return null;
            }
    ...
    protected function get_client_view_route($client_id = 0)
    {
        ...
        if ($client) {
            switch ($client->account_type) {
                case 1:
                    ...
                    break;
                case 2:
                    $route_prefix = 'migration_clients';
                    break;
                case 4:
    ...
    protected function get_location_label($location_id = 0)
    {
        $label = "-";
    ...
    protected function get_account_label($account_id = 0)
    {
        $label = "-";
        $type = $this->Account_types_model->get_one($account_id);
        if ($type) {
            $label = $type->title;
        }
        return $label;
    ...
    protected function get_client_full_name($client_id = 0, $client = null, $spacer = " ")
    {
        $full_name = "";
        if (!$client) {
            $client = $this->Clients_model->get_one($client_id);
    ...
    protected function get_clients_dropdown()
    {
        $clients_dropdown = array();
    ...
    protected function is_project_doc_check_list_item($project_check_list_id = 0)
    {
        $check_list = $this->Project_doc_check_list_model->get_one($project_check_list_id);
        if ($check_list) {
            return $check_list;
        } else {
            return false;
    ...
    protected function get_project_doc_check_list_item_dropdown($project_id = 0)
    {
        $dropdown_list = array('' => '-');
        if ($project_id) {
            $options = array('project_id' => $project_id);
            $list = $this->Project_doc_check_list_model->get_details($options)->getResult();

            foreach ($list as $item) {
                $dropdown_list[$item->id] = $item->label;
            }
        }
    ...
    protected function get_client_doc_check_list_item_dropdown($client_id = 0)
    {
        $dropdown_list = array('' => '-');
        if ($client_id) {
    ...
    protected function get_remaining_project_doc_check_list_count($project_id = 0)
    {
        if ($project_id) {
            $options = array('project_id' => $project_id, 'file_id' => 0, 'is_required' => 1);
            $list = $this->Project_doc_check_list_model->get_all_where($options)->getResult();
    ...
    protected function make_doc_check_list_dropdown()
    {
        $dropdown_list = array();
        if (!is_dev_mode()) {
            $dropdown_list = array('' => '-');
        }
        $options = array('file_type' => 'document_check_list');
        $list = $this->General_files_model->get_details($options)->getResult();
    ...
    protected function make_partners_dropdown($partner_type = '', $add_empty_row = false, $filter_label = '')
    {
        ...
        foreach ($list as $item) {
            $dropdown_list[$item->id] = $item->first_name . ' ' . $item->last_name;
        }
    ...
    protected function get_account_types_dropdown_for_filter()
    {
        ...
        $options = array();
        $list = $this->Account_types_model->get_details($options)->getResult();
        foreach ($list as $item) {
            $dropdown_list[] = array('id' => $item->id, 'text' => $item->title);;
        }
        return $dropdown_list;
    ...
    protected function make_type_dropdown($first_item_label = '')
    {
        if ($first_item_label) {
    ...
    protected function make_phases_dropdown()
    {
        ...
        $phases_dropdown = [];
        foreach ($list as $key => $value) {
            $phases_dropdown[] = array("id" => $key, "text" => $value);
        }

        return $phases_dropdown;
    }

    protected function make_jump_to_page_dropdown($trigger)
    {
        $pages = 0;
        switch ($trigger) {
            case 'students':
        ...
        $jtp_dropdown = [];
        $jtp_dropdown[] = array("id" => '', "text" => "- Jump to page -");
        foreach ($pages as $key => $value) {
            $jtp_dropdown[] = array("id" => $value, "text" => $value);
        }
    ...
    protected function get_team_members_dropdown($is_filter = false, $optional_label = '')
    {
        $team_members = $this->Users_model->get_all_where(array("user_type" => "staff", "deleted" => 0, "status" => "active"))->getResult();
        if (is_dev_mode()) {
            $inactive_team_members = $this->Users_model->get_all_where(array("user_type" => "staff", "deleted" => 0, "status" => "inactive"))->getResult();
            $team_members = array_merge($team_members, $inactive_team_members);
        }
        ...
        if ($optional_label) {
            $team_members_dropdown = array(array("id" => "", "text" => "- " . $optional_label . " -"));
        }
        ...
        foreach ($team_members as $member) {
            $_location_ids = get_tm_ol_id($member->id);
            if ((bool)array_intersect($location_ids, $_location_ids)) {
    ...
    protected function get_countries_dropdown($options)
    {
    ...
    protected function get_languages_dropdown($options)
    {
        $languages = $this->Languages_model->get_details($options)->getResult();
    ...
    protected function get_role_label($role_id = 0, $is_admin = false)
    {
        $label = "";
        if ($role_id == 0 && $is_admin) {
            $label = app_lang('admin');
        } elseif ($role_id == 0 && !$is_admin) {
            $label = app_lang('team_member');
        } elseif ($role_id && !$is_admin) {
