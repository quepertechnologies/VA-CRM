...
class Organizations extends Security_Controller
{
    ...
    function index($tab = "")
    {
        ...
        return $this->template->rander("organizations/index", $view_data);
    }
    ...
    function sub_users_modal_form()
    {
        ...
        return $this->template->view('organizations/sub_users/modal_form', $view_data);
    }
    ...
    function modal_form()
    {
        ...
        return $this->template->view('organizations/modal_form', $view_data);
    }
    ...
    function save()
    {
        ...
        $data = array(
            ...
            "contact_person_title" => $this->request->getPost('contact_person_title'),
            "contact_person_email" => $this->request->getPost('contact_person_email'),
            "contact_person_phone" => $this->request->getPost('contact_person_phone'),
            "address" => $this->request->getPost('address'),
            "city" => $this->request->getPost('city'),
            "state" => $this->request->getPost('state'),
            "zip" => $this->request->getPost('zip'),
            "country" => $this->request->getPost('country'),
            "annual_rev" => $this->request->getPost('annual_rev'),
        ...
        if ($save_id) {
            ...
            $this->save_primary_contact($save_id);
            ...
            $timeline_data = array(
                'client_id' => $save_id,
                'title' => 'Account Created by ' . $user->first_name . ' ' . $user->last_name
            );
    ...
    function save_primary_contact($client_id = 0, $location_id = 0)
    {
        if ($client_id) {
            ...
            $pwd = $this->request->getPost("password") ? $this->request->getPost("password") : bin2hex(openssl_random_pseudo_bytes(4));
            ...
            $user_data = array(
                ...
                "phone" => $user_phone,
                "job_title" => "",
                "note" => "Primary Contact (Auto Created)",
                "created_at" => get_current_utc_time(),
                "gender" => "",
                "skype" => "",
                "location_id" => $location_id ? $location_id : get_ltm_opl_id(),
            );
            ...
            if (!$this->Users_model->is_email_exists($user_data["email"], 0, $client_id)) {
                ...
                ////////////// UPDATING USER ID IN CLIENTS TABLE //////////////////////////////
                $client_data = array(
                    "primary_contact_id" => $save_id,
                );
                $update_client = $this->Clients_model->ci_save($client_data, $client_id);
                ///////////////////////////////////////////////////////////////////////////////
                ...
                $save_id = $this->Users_model->ci_save($user_data, $user->primary_contact_id);
                //$client_data['primary_contact_id'] = $save_id;
            }
        }
    }
    ...
    function save_sub_user()
    {
        ...
        $patent_info = $this->Clients_model->get_one($this->request->getPost('parent_id'));
        $location_id = 0;
        if ($patent_info) {
            $location_id = $patent_info->location_id ? $patent_info->location_id : 0;
        ...
        if ($save_id) {
            ...
            $this->save_primary_contact($save_id, $location_id);
            ...
            echo json_encode(array("success" => true, "data" => $this->_row_sub_user_data($save_id), 'id' => $save_id, 'view' => $this->request->getPost('view'), 'message' => app_lang('record_saved')));
        } else {
            echo json_encode(array("success" => false, 'message' => app_lang('error_occurred')));
        }
    }
    ...
    function list_sub_users_data($client_id = 0)
    {
        $this->access_only_allowed_members();
        ...
        foreach ($list_data as $data) {
            $result_data[] = $this->_make_sub_user_row($data, $custom_fields);
        }
    ...
    private function _row_sub_user_data($id)
    {
        $custom_fields = $this->Custom_fields_model->get_available_fields_for_table("clients", $this->login_user->is_admin, $this->login_user->user_type);
        $options = array(
            "id" => $id,
        ...
        return $this->_make_sub_user_row($data, $custom_fields);
    }
    ...
    private function _make_sub_user_row($data, $custom_fields)
    {
        ...
        $row_data = array(
            $data->id,
            get_client_contact_profile_link($data->id, $data->first_name . " " . $data->last_name, array(), array('caption' => $data->unique_id, 'account_type' => $data->account_type, 'check' => 'Sub')) . "<br><small>" . $account_type . "</small>",
            $created_at,
            $data->phone ?? "-",
            $data->email,
            $account_type,
        ...
        $row_data[] = modal_anchor(get_uri($route_prefix . "/modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_client'), "data-post-id" => $data->id))
            . js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_client'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("organizations/delete"), "data-action" => "delete-confirmation"));
    ...
    private function _make_row($data, $custom_fields)
    {
        ...
        $row_data[] = modal_anchor(get_uri("organizations/modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_client'), "data-post-id" => $data->id))
            . js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_client'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("organizations/delete"), "data-action" => "delete-confirmation"));
    ...
    function view($client_id = 0, $tab = "")
    {
        ...
        if ($client_id) {
            ...
            if ($client_info && !$client_info->is_lead) {
                ...
                $tasksOptions = array('client_id' => $client_id);
                ...
                return $this->template->rander("organizations/view", $view_data);
            } else {
                show_404();
            }
        } else {
    ...
    function add_remove_star($client_id, $type = "add")
    {
        if ($client_id) {
            ...
            if ($type === "add") {
                ...
                return $this->template->view('organizations/star/starred', $view_data);
            } else {
                $this->Clients_model->add_remove_star($client_id, $this->login_user->id, $type = "remove");
                return $this->template->view('organizations/star/not_starred', $view_data);
            }
        }
    }
    ...
    function show_my_starred_clients()
    {
        ...
        return $this->template->view('organizations/star/clients_list', $view_data);
    }
    ...
    function projects($client_id)
    {
        ...
        return $this->template->view("organizations/projects/index", $view_data);
    }
    ...
    function payments($client_id)
    {
        ...
        if ($client_id) {
            ...
            $view_data['client_id'] = clean_data($client_id);
            return $this->template->view("organizations/payments/index", $view_data);
        }
    }
    ...
    function tickets($client_id)
    {
        ...
        if ($client_id) {
            ...
            return $this->template->view("organizations/tickets/index", $view_data);
        }
    }
    ...
    function invoices($client_id)
    {
        ...
        if ($client_id) {
            ...
            return $this->template->view("organizations/invoices/index", $view_data);
        }
    }
    ...
    function estimates($client_id)
    {
        ...
        if ($client_id) {
            ...
            return $this->template->view("organizations/estimates/estimates", $view_data);
        }
    }
    ...
    function orders($client_id)
    {
        ...
        if ($client_id) {
            ...
            return $this->template->view("organizations/orders/orders", $view_data);
        }
    }
    ...
    function estimate_requests($client_id)
    {
        ...
        if ($client_id) {
            $view_data['client_id'] = clean_data($client_id);
            return $this->template->view("organizations/estimates/estimate_requests", $view_data);
        }
    }
    ...
    function notes($client_id)
    {
        ...
        if ($client_id) {
            $view_data['client_id'] = clean_data($client_id);
            return $this->template->view("organizations/notes/index", $view_data);
        }
    }
    ...
    function files($client_id, $view_type = "")
    {
        ...
        if ($view_type == "page_view") {
            ...
            return $this->template->rander("organizations/files/index", $view_data);
        } else {
            return $this->template->view("organizations/files/index", $view_data);
        }
    }
    ...
    function save_checklist()
    {
        ...
        if ($doc_check_list_id) {
            ...
            $doc_check_list = $this->General_files_model->get_details(array('id' => $doc_check_list_id))->getRow();
            if ($doc_check_list) {
            ...
            $data = array(
                "doc_check_list_id" => $doc_check_list_id,
            );

            $success = $this->Clients_model->ci_save($data, $client_id);
        } else {
            $success = false;
        }
    ...
    function check_list_data($client_id = 0)
    {
        $this->can_view_files();
        $this->_validate_client_view_access($client_id);
    ...
    function _make_checklist_row($data, $client_id = 0)
    {
        $options = array('doc_check_list_item_id' => $data->id, 'client_id' => $client_id);
        $files = $this->General_files_model->get_details($options)->getResult();

        $options = modal_anchor(get_uri("organizations/file_modal_form"), "<i data-feather='plus-circle' class='icon-16'></i> ", array("class" => "btn btn-default btn-sm", "title" => app_lang('add_file') . ": " . $data->label, "data-post-client_id" => $client_id, "data-post-doc_check_list_id" => $data->id, "data-post-doc_check_list_item" => $data->label));
    ...
    {
        $view_data['checklist_dropdown'] = $this->make_doc_check_list_dropdown();
        ...
        return $this->template->view('organizations/files/checklist_modal_form', $view_data);
    }
    ...
    function file_modal_form()
    {
        ...
        if ($this->request->getPost('full_check_list') == '1') {
            $doc_check_list = $this->get_client_doc_check_list_item_dropdown($client_id);
        ...
        $view_data['client_info'] = $this->Clients_model->get_one($client_id);
        return $this->template->view('organizations/files/modal_form', $view_data);
    }
        ...
        if ($files && get_array_value($files, 0)) {
            foreach ($files as $file) {
                ...
                if ($file_info) {
                    ...
                    $timeline_data = array(
                        'client_id' => $client_id,
                        'title' => 'New document uploaded by ' . $user->first_name . ' ' . $user->last_name,
                        'caption' => $this->request->getPost('description_' . $file),
                    );
                    $this->Timeline_model->ci_save($timeline_data);
    ...
    private function _make_file_row($data)
    {
        ...
        $description = "<div class='float-start'>" .
            js_anchor(remove_file_prefix($data->file_name), array('title' => "", "data-toggle" => "app-modal", "data-sidebar" => "0", "data-url" => get_uri("organizations/view_file/" . $data->id)));
        ...
        $options = anchor(get_uri("organizations/download_file/" . $data->id), "<i data-feather='download-cloud' class='icon-16'></i>", array("title" => app_lang("download")));

        if ($this->login_user->user_type == "staff") {
            $options .= js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_file'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("organizations/delete_file"), "data-action" => "delete-confirmation"));
        }
    ...
    function view_file($file_id = 0)
    {
        ...
        if ($file_info) {
            ...
            return $this->template->view("organizations/files/view", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function contact_profile($contact_id = 0, $tab = "")
    {
        ...
        if ($view_data['user_info']->user_type === "client") {
            ...
            return $this->template->rander("organizations/contacts/view", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function my_preferences()
    {
        ...
        return $this->template->view("organizations/contacts/my_preferences", $view_data);
    }
    ...
    function contacts($client_id = 0)
    {
        ...
        return $this->template->view("organizations/contacts/index", $view_data);
    }

    /* load users tab  */

    function sub_users($client_id = 0)
    {
        $this->access_only_allowed_members();
        ...
        return $this->template->view("organizations/sub_users/index", $view_data);
    }
    ...
    function activity($client_id = 0)
    {
        ...
        $view_data['can_edit_clients'] = $this->can_edit_clients();
        $timeline_data = array('client_id' => $client_id);
        $view_data['timeline_data'] = $this->Timeline_model->get_details($timeline_data)->getResult();

        return $this->template->view("organizations/activity/index", $view_data);
    }
    ...
    function emails($client_id = 0)
    {
        ...
        return $this->template->view("organizations/emails/index", $view_data);
    }

    function emails_data($client_id = 0)
    {
        $list_data = array();
        $result = array();
        foreach ($list_data as $data) {
    ...
    function whatsapp($client_id = 0)
    {
        ...
        return $this->template->view("organizations/whatsapp/index", $view_data);
    }
    ...
    function add_new_contact_modal_form()
    {
        ...
        return $this->template->view('organizations/contacts/modal_form', $view_data);
    }
    ...
    function contact_general_info_tab($contact_id = 0)
    {
        if ($contact_id) {
            ...
            return $this->template->view('organizations/contacts/contact_general_info_tab', $view_data);
        }
    }
    ...
    function save_profile_image($user_id = 0)
    {
        ...
        if ($profile_image) {
            $profile_image = serialize(move_temp_file("avatar.png", get_setting("profile_image_path"), "", $profile_image, "", "", false, 0, 'local'));
        ...
        if ($_FILES) {
            ...
            if ($image_file_name) {
                ...
                $profile_image = serialize(move_temp_file("avatar.png", get_setting("profile_image_path"), "", $image_file_name, "", "", false, $image_file_size, 'local'));
    ...
    private function _make_contact_row($data, $custom_fields, $hide_primary_contact_label = false)
    {
        ...
        $contact_link = anchor(get_uri("organizations/contact_profile/" . $data->id), $full_name . $primary_contact) . $removal_request_pending;
        if ($this->login_user->user_type === "client") {
        ...
        $row_data = array(
            $user_avatar,
            $contact_link,
            anchor(get_uri("organizations/view/" . $data->client_id), $full_name),
            $data->job_title,
            $data->email,
        ...
        $row_data[] = js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_contact'), "class" => "delete", "data-id" => "$data->id", "data-action-url" => get_uri("organizations/delete_contact"), "data-action" => "delete"));
    ...
    function invitation_modal()
    {
        ...
        $view_data["client_info"] = $this->Clients_model->get_one($client_id);
        return $this->template->view('organizations/contacts/invitation_modal', $view_data);
    }
    ...
    function send_invitation()
    {
        ...
        if (send_app_mail($email, $subject, $message)) {
            $timeline_data = array(
                'client_id' => $client_id,
                'title' => 'Signup invitation received via e-mail',
            );
            $this->Timeline_model->ci_save($timeline_data);
    ...
    function users()
    {
        if ($this->login_user->user_type === "client") {
            ...
            return $this->template->rander("organizations/contacts/users", $view_data);
        }
    }
    ...
    function import_clients_modal_form()
    {
        ...
        return $this->template->view("organizations/import_clients_modal_form");
    }
    ...
    function save_client_from_excel_file()
    {
        ...
        foreach ($excel_file as $key => $value) { //rows
            ...
            $timeline_data = array(
                'client_id' => $client_save_id,
                'title' => 'Account Created via (imported) Excel file',
            );
            $this->Timeline_model->ci_save($timeline_data);
    ...
    {
        $view_data["user_info"] = $this->Users_model->get_one($this->login_user->id);
        return $this->template->view("organizations/contacts/gdpr", $view_data);
    }
    ...
    function request_my_account_removal()
    {
        if (get_setting("enable_gdpr") && get_setting("clients_can_request_account_removal")) {
            ...
            app_redirect("organizations/contact_profile/$user_id/gdpr");
        }
    }
    ...
    function expenses($client_id)
    {
        ...
        if ($client_id) {
            ...
            return $this->template->view("organizations/expenses/index", $view_data);
        }
    }
    ...
    function contracts($client_id)
    {
        ...
        if ($client_id) {
            ...
            return $this->template->view("organizations/contracts/contracts", $view_data);
        }
    }
    ...
    function clients_list()
    {
        ...
        return $this->template->view("organizations/clients_list", $view_data);
    }
    ...
    function proposals($client_id)
    {
        ...
        if ($client_id) {
            ...
            return $this->template->view("organizations/proposals/proposals", $view_data);
        }
    }
    ...
    function tasks($client_id)
    {
        ...
        $view_data['client_id'] = clean_data($client_id);
        return $this->template->view("organizations/tasks/index", $view_data);
    }
}
