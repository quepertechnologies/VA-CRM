...
class Clients extends Security_Controller
{
    ...
    function save_created_by_location()
    {
        ...
        foreach ($clients as $client) {
            $_data = array(
                'created_by_location_id' => $client->location_id
            );

            $this->Clients_model->ci_save($_data, $client->id);
        }

        echo json_encode(array("success" => true, 'message' => app_lang('record_updated')));
    }

    function save_default_location($location_id)
    {
    ...
    private function _make_contact_row($data, $custom_fields, $hide_primary_contact_label = false)
    {
        ...
        $row_data = array(
            $user_avatar,
            $contact_link,
            anchor(get_uri("clients/view/" . $data->client_id), $full_name),
            $data->job_title,
            $data->email,
