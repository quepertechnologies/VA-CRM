...
class Contracts extends Security_Controller
{
    ...
    private function get_contract_clients_and_leads_dropdown()
    {
        ...
        foreach ($clients as $client) {
            $full_name = $this->get_client_full_name($client->id);
            $company_name = $client->is_lead ? (app_lang("lead") . ": " . $full_name) : (app_lang("client") . ": " . $full_name);
    ...
    function save()
    {
        ...
        if ($contract_id) {
            ...
            $_link = anchor(get_uri("contracts/view/" . $contract_id), "Contract #" . $contract_id);
            $timeline_data = array(
    ...
    private function _make_row($data, $custom_fields)
    {
        ...
        $client = anchor(get_uri("clients/view/" . $data->client_id), isset($data->company_name) ? $data->company_name : $data->first_name . " " . $data->last_name);
        if ($data->is_lead) {
            $client = anchor(get_uri("leads/view/" . $data->client_id), isset($data->company_name) ? $data->company_name : $data->first_name . " " . $data->last_name);
        }
    ...
    function preview($contract_id = 0, $show_close_preview = false, $is_editor_preview = false)
    {
        ...
        if ($contract_id) {
            ...
            $view_data['content'] = $contract_data['contract_info']->content;
    ...
    function get_contracts_dropdown($skip_default_template = false)
    {

        $options = array();
        $Contract_templates_model = model("App\Models\Contract_templates_model");
        $list_data = $Contract_templates_model->get_details($options)->getResult();

        $dropdown = array('' => '-');

        foreach ($list_data as $item) {
            $dropdown[$item->id] = get_contract_id($item->id) . ' ' . $item->title;
        }

        if ($skip_default_template) {
            $view_data['contracts_dropdown'] = array_filter($dropdown, function ($k) {
                return $k !== 1;
            }, ARRAY_FILTER_USE_KEY);
        } else {
            $view_data['contracts_dropdown'] = $dropdown;
        }

        return $view_data;
    }

    function send_project_contract_modal_form($client_id = 0)
    {
        $this->access_only_allowed_members();

        $view_data = $this->get_contracts_dropdown(true);

        $is_lead = $this->request->getPost('is_lead');
        if ($is_lead) {
            $contacts_options = array("user_type" => "lead", "client_id" => $client_id);
        } else {
            $contacts_options = array("user_type" => "client", "client_id" => $client_id);
        }
        ...
        return $this->template->view('contracts/send_project_contract_modal_form', $view_data);
    }
    ...
    function get_send_contract_template($contract_id = 0, $contact_id = 0, $return_type = "", $contract_info = "", $contact_info = "")
    {
        ...
        if ($return_type == "json") {
            echo json_encode(array("success" => true, "message_view" => $message, "subject_view" => $subject));
        } else {
            return array(
                "message" => $message,
                "subject" => $subject
    ...
    function send_contract()
    {
        ...
        $this->validate_submitted_data(array(
            ...
            'file_names' => "Please upload you contract document"
        ));
        ...
        // $Contract_templates_model = model("App\Models\Contract_templates_model");
        // $template = $Contract_templates_model->get_one($template_id);

        // $target_path = get_setting("contract_file_path");
        // var_dump($target_path);
        // exit();
        $files_data = move_files_from_temp_dir_to_permanent_dir('files/contracts/', "contract", 'local');
        $new_files = unserialize($files_data);
        // var_dump($new_files);
        // exit();
        // $timeline_file_path = get_setting("timeline_file_path");
        $new_files = update_saved_files('files/contracts/', serialize(array()), $new_files);

        $title = "APPLICATION#" . $project_id;
        // $file_name = "";

        // if ($new_files && get_array_value($new_files, 0)) {
        //     $file_name = basename(get_array_value($new_files[0], 'file_name'));
        //     $title = pathinfo($file_name);
        // }
        ...
        // $contract_data = get_contract_making_data($contract_id);
        // $contract_url = array(prepare_contract_pdf($contract_data));

        // var_dump(get_array_value($new_files[0], 'file_name'));
        // exit();
        $contract_url = array(getcwd() . '/' . 'files/contracts/' . get_array_value($new_files[0], 'file_name'));
        ...
        if (send_hellosign_sign_request($signers, $subject, $subject, $message, $cc_arr, $contract_url, $metadata) || is_server_localhost()) {
            // change email status
            $activity_data = array(
                'status' => 'sent',
                'subject' => $title,
                'send_to_name' => $full_name,
            ...
            if ($this->Contract_activity_model->ci_save($activity_data)) {
