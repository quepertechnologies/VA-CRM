...
class Students extends Security_Controller
{
    ...
    function index($tab = "")
    {
        ...
        return $this->template->rander("students/index", $view_data);
    }
    ...
    function modal_form()
    {
        ...
        return $this->template->view('students/modal_form', $view_data);
    }
    ...
    function save()
    {
        ...
        $data = array(
            ...
            "student_onshore" => $student_onshore,
            ...
            "co_phone_code" => $this->request->getPost('co_phone_code'),
            "co_phone" => $this->request->getPost('co_phone'),
            "passport_number" => $this->request->getPost('passport_number'),
            "visa_phase" => $this->request->getPost('visa_phase'),
            "visa_sub_phase" => $this->request->getPost('visa_sub_phase'),
            "lvl_of_edu" => $this->request->getPost('lvl_of_edu'),
            "country_of_citizenship" => $this->request->getPost('country_of_citizenship'),
            "result_of_study" => $this->request->getPost('result_of_study'),
            "area_of_study" => $this->request->getPost('area_of_study'),
            "intended_course_level" => $this->request->getPost('intended_course_level'),
            "course_fields_comments" => $this->request->getPost('course_fields_comments'),
            "career_paths" => $this->request->getPost('career_paths'),
            "intended_institutions" => $this->request->getPost('intended_institutions'),
            "intended_intake_quarter" => $this->request->getPost('intended_intake_quarter'),
            "intended_intake_year" => $this->request->getPost('intended_intake_year'),
            "intended_intake_comments" => $this->request->getPost('intended_intake_comments'),
            "funding_source" => $this->request->getPost('funding_source'),
            "intended_dest_1" => $this->request->getPost('intended_dest_1'),
            "intended_dest_2" => $this->request->getPost('intended_dest_2'),
            "intended_dest_3" => $this->request->getPost('intended_dest_3'),
            "intended_dest_comments" => $this->request->getPost('intended_dest_comments'),
    ...
    function save_primary_contact($client_id = 0)
    {
        if ($client_id) {
            $user_phone = "";
    ...
    function view($client_id = 0, $tab = "")
    {
        ...
        if ($client_id) {
            ...
            if ($client_info && !$client_info->is_lead) {
                ...
                return $this->template->rander("students/view", $view_data);
            } else {
                show_404();
            }
        } else {
    ...
    function add_remove_star($client_id, $type = "add")
    {
        if ($client_id) {
            ...
            if ($type === "add") {
                ...
                return $this->template->view('students/star/starred', $view_data);
            } else {
                $this->Clients_model->add_remove_star($client_id, $this->login_user->id, $type = "remove");
                return $this->template->view('students/star/not_starred', $view_data);
            }
        }
    }
    ...
    function show_my_starred_clients()
    {
        ...
        return $this->template->view('students/star/clients_list', $view_data);
    }
    ...
    function projects($client_id)
    {
        ...
        return $this->template->view("students/projects/index", $view_data);
    }
    ...
    function payments($client_id)
    {
        ...
        if ($client_id) {
            ...
            $view_data['client_id'] = clean_data($client_id);
            return $this->template->view("students/payments/index", $view_data);
        }
    }
    ...
    function tickets($client_id)
    {
        ...
        if ($client_id) {
            ...
            return $this->template->view("students/tickets/index", $view_data);
        }
    }
    ...
    function invoices($client_id)
    {
        ...
        if ($client_id) {
            ...
            return $this->template->view("students/invoices/index", $view_data);
        }
    }
    ...
    function estimates($client_id)
    {
        ...
        if ($client_id) {
            ...
            return $this->template->view("students/estimates/estimates", $view_data);
        }
    }
    ...
    function orders($client_id)
    {
        ...
        if ($client_id) {
            ...
            return $this->template->view("students/orders/orders", $view_data);
        }
    }
    ...
    function estimate_requests($client_id)
    {
        ...
        if ($client_id) {
            $view_data['client_id'] = clean_data($client_id);
            return $this->template->view("students/estimates/estimate_requests", $view_data);
        }
    }
    ...
    function notes($client_id)
    {
        ...
        if ($client_id) {
            $view_data['client_id'] = clean_data($client_id);
            return $this->template->view("students/notes/index", $view_data);
        }
    }
    ...
    function files($client_id, $view_type = "")
    {
        ...
        if ($view_type == "page_view") {
            ...
            return $this->template->rander("students/files/index", $view_data);
        } else {
            return $this->template->view("students/files/index", $view_data);
        }
    }
    ...
    function _make_checklist_row($data, $client_id = 0)
    {
        ...
        $options = modal_anchor(get_uri("students/file_modal_form"), "<i data-feather='plus-circle' class='icon-16'></i> ", array("class" => "btn btn-default btn-sm", "title" => app_lang('add_file') . ": " . $data->label, "data-post-client_id" => $client_id, "data-post-doc_check_list_id" => $data->id, "data-post-doc_check_list_item" => $data->label));
    ...
    function checklist_modal_form()
    {
        ...
        return $this->template->view('students/files/checklist_modal_form', $view_data);
    }
    ...
    function file_modal_form()
    {
        ...
        $view_data['client_info'] = $this->Clients_model->get_one($client_id);
        return $this->template->view('students/files/modal_form', $view_data);
    }
    ...
    private function _make_file_row($data)
    {
        ...
        $description = "<div class='float-start'>" .
            js_anchor(remove_file_prefix($data->file_name), array('title' => "", "data-toggle" => "app-modal", "data-sidebar" => "0", "data-url" => get_uri("students/view_file/" . $data->id)));
        ...
        $options = anchor(get_uri("students/download_file/" . $data->id), "<i data-feather='download-cloud' class='icon-16'></i>", array("title" => app_lang("download")));

        if ($this->login_user->user_type == "staff") {
            $options .= js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_file'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("students/delete_file"), "data-action" => "delete-confirmation", 'data-reload-on-success' => '1'));
        }
    ...
    function view_file($file_id = 0)
    {
        ...
        if ($file_info) {
            ...
            return $this->template->view("students/files/view", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function contact_profile($contact_id = 0, $tab = "")
    {
        ...
        if ($view_data['user_info']->user_type === "client") {
            ...
            return $this->template->rander("students/contacts/view", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function my_preferences()
    {
        ...
        return $this->template->view("students/contacts/my_preferences", $view_data);
    }
    ...
    function contacts($client_id = 0)
    {
        ...
        return $this->template->view("students/contacts/index", $view_data);
    }
    ...
    function activity($client_id = 0)
    {
        ...
        return $this->template->view("students/activity/index", $view_data);
    }
    ...
    function emails($client_id = 0)
    {
        ...
        return $this->template->view("students/emails/index", $view_data);
    }
    ...
    function whatsapp($client_id = 0)
    {
        ...
        return $this->template->view("students/whatsapp/index", $view_data);
    }
    ...
    function add_new_contact_modal_form()
    {
        ...
        return $this->template->view('students/contacts/modal_form', $view_data);
    }
    ...
    function contact_general_info_tab($contact_id = 0)
    {
        if ($contact_id) {
            ...
            return $this->template->view('students/contacts/contact_general_info_tab', $view_data);
        }
    }
    ...
    private function _make_contact_row($data, $custom_fields, $hide_primary_contact_label = false)
    {
        ...
        $contact_link = anchor(get_uri("students/contact_profile/" . $data->id), $full_name . $primary_contact) . $removal_request_pending;
        if ($this->login_user->user_type === "client") {
        ...
        $row_data = array(
            $user_avatar,
            $contact_link,
            anchor(get_uri("students/view/" . $data->client_id), $full_name),
            $data->job_title,
            $data->email,
        ...
        $row_data[] = js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_contact'), "class" => "delete", "data-id" => "$data->id", "data-action-url" => get_uri("students/delete_contact"), "data-action" => "delete"));
    ...
    function invitation_modal()
    {
        ...
        $view_data["client_info"] = $this->Clients_model->get_one($client_id);
        return $this->template->view('students/contacts/invitation_modal', $view_data);
    }
    ...
    function users()
    {
        if ($this->login_user->user_type === "client") {
            ...
            return $this->template->rander("students/contacts/users", $view_data);
        }
    }
    ...
    function import_clients_modal_form()
    {
        ...
        return $this->template->view("students/import_clients_modal_form");
    }
    ...
    {
        $view_data["user_info"] = $this->Users_model->get_one($this->login_user->id);
        return $this->template->view("students/contacts/gdpr", $view_data);
    }
    ...
    function request_my_account_removal()
    {
        if (get_setting("enable_gdpr") && get_setting("clients_can_request_account_removal")) {
            ...
            app_redirect("students/contact_profile/$user_id/gdpr");
        }
    }
    ...
    function expenses($client_id)
    {
        ...
        if ($client_id) {
            ...
            return $this->template->view("students/expenses/index", $view_data);
        }
    }
    ...
    function contracts($client_id)
    {
        ...
        if ($client_id) {
            ...
            return $this->template->view("students/contracts/contracts", $view_data);
        }
    }
    ...
    function clients_list()
    {
        ...
        return $this->template->view("students/clients_list", $view_data);
    }
    ...
    function proposals($client_id)
    {
        ...
        if ($client_id) {
            ...
            return $this->template->view("students/proposals/proposals", $view_data);
        }
    }
    ...
    function tasks($client_id)
    {
        ...
        $view_data['client_id'] = clean_data($client_id);
        return $this->template->view("students/tasks/index", $view_data);
    }
}
