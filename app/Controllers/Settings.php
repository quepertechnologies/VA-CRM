...
class Settings extends Security_Controller
{
    ...
    {
        $settings = array("site_logo", "favicon", "show_background_image_in_signin_page", "show_logo_in_signin_page", "app_title", "accepted_file_formats", "landing_page", "rows_per_page", "item_purchase_code", "scrollbar", "enable_rich_text_editor", "show_theme_color_changer", "default_theme_color", 'enable_dev_mode');
        $has_php_file_format = false;

        foreach ($settings as $setting) {
            $value = $this->request->getPost($setting);

            if ($setting == 'enable_dev_mode') {
                $this->Settings_model->save_setting($setting, $value ? '1' : '0'); //can be saved as blank also
            } elseif ($setting == "landing_page") {
                $this->Settings_model->save_setting($setting, $value); //can be saved as blank also
            } else if ($value || $value === "0") {
                if ($setting === "site_logo") {
                    $value = str_replace("~", ":", $value);
                    $value = serialize(move_temp_file("site-logo.png", get_setting("system_file_path"), "", $value, "", "", false, 0, 'app_storage'));
                    ...
                    $value = str_replace("~", ":", $value);
                    $value = serialize(move_temp_file("favicon.png", get_setting("system_file_path"), "", $value, "", "", false, 0, 'app_storage'));
        ...
        if ($_FILES) {
            ...
            foreach ($files as $file) {
                ...
                $new_file_data = serialize(move_temp_file($new_file_name, get_setting("system_file_path"), "", $file_name, "", "", false, $file_size, 'app_storage'));
                //delete old file
    ...
    function upload_wf_file()
    {
        upload_wf_file_to_temp();
    }
    ...
    function validate_wf_file()
    {
        return validate_workflow_file($this->request->getPost("file_name"));
    }
    ...
    /* show the dropbox hellosign settings tab */

    function dropbox_hellosign()
    {
        return $this->template->view("settings/integration/dropbox_hellosign");
    }

    /* save dropbox hellosign settings */
    function save_dropbox_hellosign_settings()
    {
        $settings = array(
            "enable_dropbox_hellosign_api",
            "dropbox_hellosign_app_client_id",
            "dropbox_hellosign_api_key",
        );
        ...
        foreach ($settings as $setting) {
            ...
            // if (get_setting("google_drive_authorized") && ($setting == "google_drive_client_id" || $setting == "google_drive_client_secret") && $enable_ms_onedrive && get_setting($setting) != $value) {
    ...
    /* show the browse ai settings tab */
    function browse_ai()
    {
        return $this->template->view("settings/integration/browse_ai");
    }

    /* save dropbox hellosign settings */
    function save_browse_ai_settings()
    {
        $settings = array(
            "enable_browse_ai_api",
            "browse_ai_api_key",
        );
    ...
    function xero_api()
    {
        return $this->template->view("settings/integration/xero_api");
    }

    /* save dropbox hellosign settings */
    function save_xero_api_settings()
    {
        $settings = array(
            "enable_xero_api",
            "xero_api_client_id",
            'xero_api_client_secret',
            'xero_api_redirect_uri',
            'xero_api_url_authorize',
            'xero_api_url_access_token',
            'xero_api_url_resource_owner_details'
        );
    ...
    /* show the fresh desk ai settings tab */
    function fresh_desk_api()
    {
        return $this->template->view("settings/integration/fresh_desk_api");
    }

    /* save dropbox hellosign settings */
    function save_fresh_desk_api_settings()
    {
        $settings = array(
    ...
    /* show the microsoft one drive settings tab */

    function ms_onedrive()
    {
        return $this->template->view("settings/integration/ms_onedrive");
    }


    /* save microsoft one drive drive settings */

    function save_ms_onedrive_settings()
    {
        $settings = array(
            "enable_ms_onedrive_api_to_upload_file",
            "ms_onedrive_client_id",
            "ms_onedrive_client_secret",
            "ms_onedrive_tenant_id",
            "ms_onedrive_redirect_uri",
            "ms_onedrive_grant_type",
            "ms_onedrive_clients_directory_id",
            "ms_onedrive_refresh_token",
            "ms_onedrive_base_directory_path",
        );

        // $enable_ms_onedrive = $this->request->getPost("enable_ms_onedrive_api_to_upload_file");
    ...
    {
        $view_data = array();
        return $this->template->rander("workflows/index", $view_data);
    }

    function document_check_lists()
    {
        $view_data = array();
        return $this->template->rander("document_check_list/index", $view_data);
    }

    function document_check_lists_data()
    {
        $this->can_access_clients();

        $options = array('file_type' => 'document_check_list', 'user_id' => $this->login_user->id);
        $list_data = $this->General_files_model->get_details($options)->getResult();
        $result = array();
        foreach ($list_data as $data) {
            $result[] = $this->_make_workflow_row($data);
        }
        echo json_encode(array("data" => $result));
    }

    function workflows_data()
    {
        $this->can_access_clients();

        $options = array('file_type' => 'workflow', 'user_id' => $this->login_user->id);
        $list_data = $this->General_files_model->get_details($options)->getResult();
        $result = array();
        foreach ($list_data as $data) {
            $result[] = $this->_make_workflow_row($data);
    ...
    private function _make_workflow_row($data)
    {
        ...
        $options = anchor(get_uri("settings/download_workflow_file/" . $data->id), "<i data-feather='download-cloud' class='icon-16'></i>", array("title" => app_lang("download")));

        if ($this->login_user->user_type == "staff") {
            $options .= js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_file'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("settings/delete_workflow_file"), "data-action" => "delete-confirmation"));
        }
    ...
    function workflow_modal_form()
    {
        ...
        return $this->template->view('workflows/modal_form', $view_data);
    }

    function document_check_list_modal_form()
    {
        $this->can_add_files();
        ...
        $view_data['client_id'] = $client_id;
        $view_data['client_info'] = $this->Clients_model->get_one($client_id);
        return $this->template->view('document_check_list/modal_form', $view_data);
    }

    /* save file data and move temp file to parmanent file directory */

    function save_workflow_file()
    {
        $this->can_add_files();
        ...
        $target_path = getcwd() . "/" . get_general_file_path("staff", $this->login_user->id);

        //process the fiiles which has been uploaded by dropzone
        if ($files && get_array_value($files, 0)) {
            foreach ($files as $file) {
                $file_name = $this->request->getPost('file_name_' . $file);
                $file_info = move_workflow_temp_file($file_name, $target_path);
                if ($file_info) {
                    $data = array(
                        "client_id" => 0,
    ...
    function save_document_check_list_file()
    {
        $this->can_add_files();
    ...
    function delete_workflow_file()
    {
        ...
        if ($info->user_id && ($this->login_user->is_admin || $this->login_user->id === $info->uploaded_by)) {
            if ($this->General_files_model->delete($id)) {
                ...
                //delete the files
                delete_app_files(get_general_file_path("staff", $info->user_id), array(make_array_of_file($info)));
    ...
    function download_workflow_file($id)
    {
        ...
        return $this->download_app_files(get_general_file_path("staff", $file_info->user_id), $file_data);
    }
    ...
    {
        $view_data = array();
        return $this->template->rander("settings/locations/index", $view_data);
    }

    function location_data()
    {
        $options = array();
        $list_data = $this->Location_model->get_details($options)->getResult();
        $result = array();
        foreach ($list_data as $data) {
            $result[] = $this->_make_location_row($data);
        }
        echo json_encode(array("data" => $result));
    }

    private function _make_location_row($data)
    {
        $options =
            modal_anchor(get_uri("settings/location_modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('location'), "data-post-id" => $data->id));

        if ($this->login_user->user_type == "staff") {
            $options .= js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_file'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("students/delete_file"), "data-action" => "delete-confirmation"));
        }
    ...
    function location_modal_form()
    {
        ...
        $view_data = array();
        if ($id) {
            $model_info = $this->Location_model->get_details(array("id" => $id))->getRow();
            $view_data["location_id"] = $id;
            $view_data["model_info"] = $model_info;
        } else {
            $view_data["location_id"] = 0;
        }

        return $this->template->view("settings/locations/modal_form", $view_data);
    }
    ...
    function save_location()
    {
        ...
        if ($location_id === 0) {
            $save_id = $this->Location_model->ci_save($data);
        } else {
            $save_id = $this->Location_model->ci_save($data, $location_id);
        }
        ...
        if ($save_id) {
            ...
            // if ($location_id === 0) {
            // }
            $model_info = $this->Location_model->get_details(array("id" => $save_id))->getRow();
            $data = $this->_make_location_row($model_info);
            echo json_encode(array("success" => true, 'message' => app_lang('record_saved'), 'id' => $save_id, 'data' => $data, 'newData' => $location_id === 0 ? true : false));
        } else {
            echo json_encode(array("success" => false, 'message' => app_lang('error_occurred')));
        }
    }
