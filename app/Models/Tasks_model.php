...
class Tasks_model extends Crud_model
{
    ...
    function __construct()
    {
        ...
        parent::init_activity_log("task", "title", "project", "project_id", '', 0, 'milestone', 'milestone_id');
    }
    ...
    function get_details($options = array())
    {
        ...
        if ($specific_user_id) {
            $where .= " AND $tasks_table.assigned_to=$specific_user_id";
        ...
        if ($show_assigned_tasks_only_user_id) {
            $where .= " AND $tasks_table.assigned_to=$show_assigned_tasks_only_user_id";
        ...
        $collaborators = $this->_get_clean_value($options, "collaborators");
        if ($collaborators) {
            $where .= " AND (FIND_IN_SET('$collaborators', $tasks_table.collaborators)) ";
        }
