...
class Users_model extends Crud_model
{
    ...
    function save_location($data)
    {
        ...
        $location_id = $this->_get_clean_value($data, "location_id");
        $data = array('location_id', $location_id);
        if ($user_id && $location_id) {
            $this->ci_save($data, $user_id);
        }
    }
    ...
    function get_access_info($user_id = 0)
    {
        ...
        $sql = "SELECT $users_table.id, $users_table.user_type, $users_table.is_admin, $users_table.role_id, $users_table.email,
            ...
            $users_table.is_primary_contact, $users_table.sticky_note, $users_table.language, $users_table.location_id,
    ...
    function count_total_contacts($options = array())
    {
        ...
        if ($location_id) {
            $where .= " AND $users_table.location_id='$location_id'";
        }

        $location_ids = $this->_get_clean_value($options, "location_ids");
        if ($location_ids) {
            $where .= " AND $users_table.location_id IN ($location_ids)";
        }
    ...
    function count_total_users($options = array())
    {
        $users_table = $this->db->prefixTable('users');
