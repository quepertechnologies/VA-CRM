...
class Projects_model extends Crud_model
{
    ...
    function get_details($options = array())
    {
        ...
        $temp_applic_id = $this->_get_clean_value($options, "temp_applic_id");
        if ($temp_applic_id) {
            $where .= " AND $projects_table.temp_applic_id='$temp_applic_id'";
        }

        $workflow_id = $this->_get_clean_value($options, "workflow_id");
        if ($workflow_id) {
            $where .= " AND $projects_table.workflow_id='$workflow_id'";
        }

        $location_id = $this->_get_clean_value($options, "location_id");
        if ($location_id) {
            $where .= " AND $projects_table.location_id='$location_id'";
        }
