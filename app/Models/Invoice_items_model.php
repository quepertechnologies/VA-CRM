...
class Invoice_items_model extends Crud_model
{
    ...
    function get_details($options = array())
    {
        ...
        if ($schedule_id) {
            $where .= " AND $invoice_items_table.schedule_id=$schedule_id";
        }
        $schedule_fee_id = $this->_get_clean_value($options, "schedule_fee_id");
        if ($schedule_fee_id) {
            $where .= " AND $invoice_items_table.schedule_fee_id=$schedule_fee_id";
        }
