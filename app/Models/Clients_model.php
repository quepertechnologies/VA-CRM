...
class Clients_model extends Crud_model
{
    ...
    function get_details($options = array())
    {
        ...
        if ($visa_phase) {
            $where .= " AND $clients_table.visa_phase='$visa_phase'";
        }

        $va_internal_id = $this->_get_clean_value($options, "va_internal_id");
        if ($va_internal_id) {
            $where .= " AND $clients_table.va_internal_id='$va_internal_id'";
        }
        ...
        $current_lead_status = $this->_get_clean_value($options, "current_lead_status");
        if ($current_lead_status) {
            $where .= " AND $clients_table.current_lead_status='$current_lead_status'";
        }

        $is_sub_user = $this->_get_clean_value($options, "is_sub_user");
        if ($is_sub_user) {
            $where .= " AND $clients_table.is_sub_user='$is_sub_user'";
        } else {
            $where .= " AND $clients_table.is_sub_user=0";
        }
    ...
    function count_total_clients($options = array())
    {
        ...
        if ($lead_status_id) {
            $where .= " AND $clients_table.lead_status_id='$lead_status_id'";
        }
