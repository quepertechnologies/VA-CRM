...
class Project_comments_model extends Crud_model
{
    ...
    function save_comment($data)
    {
        ...
        if ($this->_get_clean_value($data, "description")) {
            parent::init_activity_log("project_comment", "description", "project", "project_id", "", 0, 'milestone', 'milestone_id');
        }
        ...
        if ($comment_id) {
            ...
            parent::init_activity_log($reply_type, "description", "project", "project_id", $type, $type_id, 'milestone', 'milestone_id');
        } else if ($file_id) {
            $file_info = $this->Project_files_model->get_one($file_id);
            $data["project_id"] = $file_info->project_id;
            parent::init_activity_log("project_comment", "description", "project", "project_id", "file", "file_id", 'milestone', 'milestone_id');
        } else if ($task_id) {
            $task_info = $this->Tasks_model->get_one($task_id);
            $data["project_id"] = $task_info->project_id;

            if ($this->_get_clean_value($data, "description")) {
                parent::init_activity_log("task_comment", "description", "project", "project_id", "task", "task_id", 'milestone', 'milestone_id');
            }
        } else if ($customer_feedback_id) {
            $data["project_id"] = $customer_feedback_id;
            parent::init_activity_log("customer_feedback", "description", "project", "project_id", "customer_feedback", "customer_feedback_id", 'milestone', 'milestone_id');
        }
