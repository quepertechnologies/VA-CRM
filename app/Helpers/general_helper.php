...
if (!function_exists('send_hellosign_sign_request')) {

    /**
     * send Dropbox HelloSign Signature request
     * 
     * @param array $signers
     * @param string $title
     * @param string $subject
     ...
     * @return true/false
     */
    function send_hellosign_sign_request($signers = array(), $title = '', $subject = '', $message = '', $cc = array(), $files = array(), $metadata = null)
    {
        if (get_setting('enable_dropbox_hellosign_api')) {
            $config = Dropbox\Sign\Configuration::getDefaultConfiguration();

            // Configure HTTP basic authorization: api_key
            $config->setUsername(get_setting('dropbox_hellosign_api_key'));
            ...
            foreach ($signers as $key => $signer) {
                $request_signer = new Dropbox\Sign\Model\SubSignatureRequestSigner();
                $request_signer->setEmailAddress($signer['email'])
                    ->setName($signer['name'])
                    ->setOrder($signing_order);

                $signing_order++;

                $all_signers[] = $request_signer;
            }
            $files_to_sign = array();
            foreach ($files as $key => $file) {
                $files_to_sign[] = new SplFileObject($file);
            }
            ...
            $data->setTitle($title)
                ->setSubject($subject)
                ->setMessage($message)
                ->setSigners($all_signers)
                ->setCcEmailAddresses($cc)
                ->setFiles($files_to_sign)
            ...
            try {
                ...
                if ($result) {
                    $response = $result->getSignatureRequest();
                    if ($response) {
                        $signature_request_id = $response->getSignatureRequestId();
 ...
 * @return html link
 */
if (!function_exists('get_login_team_member_profile_link')) {

    function get_login_team_member_profile_link()
    {
        $ci = new Security_Controller(false);
        return anchor("team_members/view/" . $ci->login_user->id, $ci->login_user->first_name . ' ' . $ci->login_user->last_name, '');
    }
}
 ...
 * @return bool
 */
if (!function_exists('is_server_localhost')) {

    function is_server_localhost()
    {
...
if (!function_exists('is_dev_mode')) {
    ...
    {
        if ((int)get_setting("enable_dev_mode")) {
            return true;
        } else {
            return false;
        }
...
if (!function_exists('get_client_contact_profile_link')) {
    function get_client_contact_profile_link($id = 0, $name = "", $attributes = array(), $options = array())
    {
        $account_type = 1;
        $caption = '';
        $check = '';
        ...
        if (array_key_exists('partner_type', $options)) {
        ...
        if ($only_url) {
            return $url_prefix . "/view/" . $id . "?" . $check;
        }

        return anchor($url_prefix . "/view/" . $id . "?" . $check, $name ? $name . $caption : $caption, $attributes);
    }
}

/**
 * returns the max number form the array attribute
 * 
...
if (!function_exists('max_attribute_in_array')) {
    function max_attribute_in_array($array, $prop)
    {
        return max(array_map(
            function ($o) use ($prop) {
                return (int)get_array_value($o, $prop);
            },
            $array
        ));
    }
...
if (!function_exists("get_update_task_info_anchor_data")) {
    ...
    function get_update_task_info_anchor_data($model_info, $type = "", $can_edit_tasks = false, $extra_data = "", $extra_condition = false, $data_act = "update-task-info")
    {
        if ($model_info && $type) {
...
if (!function_exists('show_message')) {
    ...
    function show_message($title = '', $message = '')
    {
        ...
        echo view("errors/html/error_message", $data);
        exit();
    }
}
...
if (!function_exists('prepare_contract_pdf')) {
    ...
    function prepare_contract_pdf($contract_data, $mode = "send_email")
    {
        $pdf = new Pdf();
        $pdf->setPrintHeader(false);
        $pdf->setPrintFooter(false);
        ...
        if ($contract_data) {
            ...
            $html = prepare_contract_view($contract_data);
...
if (!function_exists('get_ltm_opl_id')) {

    /**
     * get login team member office primary location id
     * 
     * @return string|int|array
     */
    function get_ltm_opl_id($only_primary_location = true, $merge_by = null, $allow_conditions = true)
    {
        $ci = new Security_Controller(false);

        $options = array('user_id' => $ci->login_user->id);

        if ($only_primary_location) {
            // if ($ci->login_user->location_id == 0) {
            //     return 0;
            // }
            $options['is_primary'] = 1;
            $office = $ci->Office_model->get_details($options)->getRow();
            ...
            if ($allow_conditions) {
                if ((int)$ci->login_user->location_id !== 0 && !$merge_by) {
                    return array($ci->login_user->location_id);
                } elseif ((int)$ci->login_user->location_id !== 0 && $merge_by) {
                    return implode($merge_by, array($ci->login_user->location_id));
                }
            }

            if ($merge_by) {
                array_push($location_ids, (int)$ci->login_user->location_id);
                return implode($merge_by, array_unique($location_ids));
            }
            return $location_ids;
        }
...
if (!function_exists('get_tm_ol_id')) {

    /**
     * get team member office location id/ids
     * 
     * @return string|int|array
     */
