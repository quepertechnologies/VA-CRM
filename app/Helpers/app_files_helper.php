 ...
 * @return void
 */
if (!function_exists('upload_wf_file_to_temp')) {

    function upload_wf_file_to_temp($upload_to_local = false)
    {
        if (!empty($_FILES)) {
            $file = get_array_value($_FILES, "file");
            ...
            if (!is_valid_workflow_file_to_upload($file_name)) {
                return false;
            }
...
if (!function_exists('move_temp_file')) {
    ...
    function move_temp_file($file_name, $target_path, $related_to = "", $source_path = NULL, $static_file_name = "", $file_content = "", $direct_upload = false, $file_size = 0, $storage = 'local')
    {
        ...
        if (defined('PLUGIN_CUSTOM_STORAGE')) {
            ...
            // if (get_setting('enable_ms_onedrive_api_to_upload_file') && $storage == 'onedrive') {
            //     if (!starts_with($source_path, "data") && !$file_content) {
            //         $uploaded = upload_to_onedrive($source_path, $target_path, $new_filename);
            //         if ($uploaded) {
            //             // $directory = get_onedrive_directory($target_path);
            //             $files_data = array("file_name" => $new_filename, 'service_type' => "onedrive", 'file_id' => $uploaded['id']);
            //         }
            //     }
            // } else {
 ...
 * @return filename
 */
if (!function_exists('move_workflow_temp_file')) {

    function move_workflow_temp_file($file_name, $target_path, $related_to = "", $source_path = NULL, $static_file_name = "", $file_content = "", $direct_upload = false, $file_size = 0)
    {
        //to make the file name unique we'll add a prefix
        $filename_prefix = $related_to . "_" . uniqid("file") . "-";
...
if (!function_exists("get_onedrive_access_token")) {

    /**
     * Request Microsoft OneDrive access token from the Azure Active Directory
     * @return string|bool access token
     */
    function get_onedrive_access_token()
    {
        $curl = curl_init();

        curl_setopt_array($curl, array(
            CURLOPT_URL => 'https://login.microsoftonline.com/' . get_setting("ms_onedrive_tenant_id") . '/oauth2/v2.0/token',
            ...
            CURLOPT_POSTFIELDS => 'refresh_token=' . get_setting('ms_onedrive_refresh_token') . '&redirect_uri=' . get_setting('ms_onedrive_redirect_uri') . '&grant_type=' . get_setting('ms_onedrive_grant_type') . '&client_secret=' . get_setting('ms_onedrive_client_secret') . '&client_id=' . get_setting('ms_onedrive_client_id'),
        ...
        if (is_string($response)) {
            ...
            if ($res) {
                $access_token = get_array_value($res, 'access_token');
                $refresh_token = get_array_value($res, 'refresh_token');
                if ($refresh_token) {
                    $ci = new App_Controller();
                    $ci->Settings_model->save_setting('ms_onedrive_refresh_token', $refresh_token);
                }
                if ($access_token) {
                    return $access_token;
                } else {
...
if (!function_exists("list_ms_onedrive_directory_items")) {
     ...
     * @return string|bool access token
     */
    function list_ms_onedrive_directory_items($source_path = "")
    {
        ...
        if ($onedrive_access_token) {
            ...
            $curl = curl_init();
            curl_setopt_array($curl, array(
                CURLOPT_URL => 'https://graph.microsoft.com/v1.0/drives/' . get_setting('ms_onedrive_clients_directory_id') . '/root:/' . $source_path . ':/children',
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_ENCODING => '',
                CURLOPT_MAXREDIRS => 10,
                CURLOPT_TIMEOUT => 0,
...
if (!function_exists("get_client_drive_items")) {

    /**
     * Get client drive items
     * @return array|bool access token
     */
    function get_client_drive_items($directory_path = "")
    {
        $json = list_ms_onedrive_directory_items("Clients/" . $directory_path);
        if ($json) {
...
if (!function_exists("upload_to_onedrive")) {

    /**
     * Upload the buffer stream to the OneDrive
     * @return string|bool access token
     */
    function upload_to_onedrive($source_path = "", $target_path = "", $filename = "")
    {
        $pointer_size = 0;
        $pointer = fopen($source_path, 'r+');
        $mime = mime_content_type($source_path);
        if ($pointer) {
            $stat = fstat($pointer);
            $pointer_size = $stat['size'];
        }

        $access_token = get_onedrive_access_token();
        $directory = get_onedrive_directory($target_path);

        if ($pointer && $mime && $access_token) {
            $ch = curl_init('https://graph.microsoft.com/v1.0/me/drive/root:/' . $directory . '/' . $filename . ':/content');
            $authorization = "Authorization: Bearer " . $access_token; // Prepare the authorization token

            curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'PUT');
            curl_setopt($ch, CURLOPT_PUT, TRUE);
            curl_setopt($ch, CURLOPT_INFILE, $pointer);
            curl_setopt($ch, CURLOPT_INFILESIZE, (int)$pointer_size);
            ...
            curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: ' . $mime, $authorization));
...
if (!function_exists("get_onedrive_item_preview_embeddable_link")) {

    /**
     * Get the Microsoft OneDrive item preview embeddable link
     * @return mixed|bool response
     */
    function get_onedrive_item_preview_embeddable_link($file_id = "")
    {
        $onedrive_access_token = get_onedrive_access_token();
        if ($file_id && $onedrive_access_token) {
            $authorization = "Authorization: Bearer " . $onedrive_access_token; // Prepare the authorization token
            $curl = curl_init();

            curl_setopt_array($curl, array(
                CURLOPT_URL => 'https://graph.microsoft.com/v1.0/me/drive/items/' . $file_id . '/preview',
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_ENCODING => '',
                CURLOPT_MAXREDIRS => 10,
                CURLOPT_TIMEOUT => 0,
...
if (!function_exists("delete_ms_onedrive_item")) {

    /**
     * Deletes Microsoft OneDrive item by the provided file id
     * @return mixed|bool response
     */
    function delete_ms_onedrive_item($file_id = "")
    {
        $onedrive_access_token = get_onedrive_access_token();
        if ($file_id && $onedrive_access_token) {
            $authorization = "Authorization: Bearer " . $onedrive_access_token; // Prepare the authorization token
            ...
            curl_setopt_array($curl, array(
                CURLOPT_URL => 'https://graph.microsoft.com/v1.0/me/drive/items/' . $file_id,
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_ENCODING => '',
                CURLOPT_MAXREDIRS => 10,
                CURLOPT_TIMEOUT => 0,
            ...
            // $response = curl_exec($curl); //? no data is returned upon file deletion
            $httpcode = curl_getinfo($curl, CURLINFO_HTTP_CODE);
            curl_close($curl);
 ...
 */
if (!function_exists("get_onedrive_directory")) {

    function get_onedrive_directory($target_path = "")
    {
        $directory = "";
        $path_chunks = array();
        if (strpos($target_path, '/files/')) {
            $path_chunks = explode('/files/', $target_path);
        } elseif (strpos($target_path, '\\files\\')) {
            $path_chunks = explode('\\files\\', $target_path);
        }

        if (array_key_exists(1, $path_chunks)) {
            $directory = $path_chunks[1];
        }
...
if (!function_exists('get_source_url_of_file')) {
    ...
    function get_source_url_of_file($file_info = array(), $file_path = "", $view_type = "", $only_file_path = false, $add_slash = false, $show_full_size_thumbnail = false)
    {
        ...
        if ($service_type && $service_type !== "google" && $service_type !== 'onedrive') {
            try {
        ...
        } else if ($service_type === 'onedrive') {
            //microsoft onedrive file
            return get_source_url_of_ms_onedrive_file($file_id, $view_type, $show_full_size_thumbnail);
        } else {
            //local file
 ...
 * @return source url
 */
if (!function_exists("get_source_url_of_ms_onedrive_file")) {

    function get_source_url_of_ms_onedrive_file($file_id = "", $view_type = "", $show_full_size_thumbnail = false)
    {
        $res = get_onedrive_item_preview_embeddable_link($file_id);
        if ($res) {
            return $res['getUrl'];
        } else {
            // needs user authorization with one drive
            return get_setting('ms_onedrive_base_directory_path') . '/' . $file_id;
        }
    }
}
...
if (!function_exists('move_files_from_temp_dir_to_permanent_dir')) {
    ...
    function move_files_from_temp_dir_to_permanent_dir($target_path = "", $related_to = "", $storage = 'onedrive')
    {
        ...
        if ($file_names && get_array_value($file_names, 0)) {
            foreach ($file_names as $key => $file_name) {
                ...
                $file_data = move_temp_file($file_name, $target_path, $related_to, null, "", "", false, $file_size, $storage);
                $files_data[] = array(
 ...
 * @return json data of success or error message
 */
if (!function_exists('validate_workflow_file')) {

    function validate_workflow_file($file_name = "")
    {
        if (!ini_get('file_uploads')) {
            echo json_encode(array("success" => false, 'message' => app_lang('please_enable_the_file_uploads_php_settings')));
            exit();
 ...
 * @return true/false
 */
if (!function_exists('is_valid_workflow_file_to_upload')) {

    function is_valid_workflow_file_to_upload($file_name = "")
    {
 ...
 * @return string of file path
 */
if (!function_exists('get_logs_file_path')) {

    function get_logs_file_path($context, $is_localhost_path = false)
    {
        if ($context) {
            $target_path = 'files/logs/';

            if (is_server_localhost() || $is_localhost_path) {
                $target_path = 'visaalliance-crm/' . $target_path;
            }
