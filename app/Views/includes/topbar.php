                        ...
                        <li class="nav-item dropdown">
                            ...
                            <ul class="dropdown-menu dropdown-menu-end language-dropdown">
                                <li>
                                    ...
                                    //     }
                                    // }
                                    $user_location = get_ltm_opl_id(false, null, false);

                                    foreach ($locations_dropdown as $location_key => $_location_text) {
                                        $location_status = "";
                                        $location_text = $_location_text;

                                        if (is_array($user_location) && $location_key == 0) {
                                            $locs = array();
                                            foreach ($locations_dropdown as $loc_key => $loc_text) {
                                                if (in_array($loc_key, $user_location)) {
                                                    $locs[] = $loc_text;
                                                }
                                            }
                                            $location_text = "<strong>" . $location_text . "</strong><br><small>" . implode(', ', $locs) . "</small>";
                                        }

                                        if ($login_user->location_id == $location_key) {
                                            $location_status = "<span class='float-end checkbox-checked m0'></span>";
                                        }
                                        // else if ($user_location == $location_key) {
                                        //     $location_status = "<span class='float-end checkbox-checked m0'></span>";
                                        //     $location_text = "<strong>" . $location_text . "</strong>";
                                        // }

                                        if ($login_user->user_type == "staff") {
                                            echo ajax_anchor(get_uri("team_members/save_default_location/$location_key"), $location_text . $location_status, array("class" => "dropdown-item clearfix", "data-reload-on-success" => "1"));
                                            if ($location_key === 0) {
                                                echo '<div class="dropdown-divider"></div><div style="margin-left: 8px;"><small><strong>BRANCHES</strong></small></div>';
                                            }
                                        } else {
                                            echo ajax_anchor(get_uri("clients/save_default_location/$location_key"), $location_text . $location_status, array("class" => "dropdown-item clearfix", "data-reload-on-success" => "1"));
                                            if ($location_key === 0) {
                                                echo '<div class="dropdown-divider"></div><div style="margin-left: 8px;"><small><strong>BRANCHES</strong></small></div>';
                                            }
                                        }
