            ...
            <div class="tab-title clearfix">
                ...
                <div class="title-button-group">
                    ...
                    if ($login_user->user_type == "staff" || ($login_user->user_type == "client" && get_setting("client_can_add_files"))) {
                        echo modal_anchor(get_uri("check_in/file_modal_form"), "<i data-feather='plus-circle' class='icon-16'></i> " . app_lang('add_files'), array("class" => "btn btn-default", "title" => app_lang('add_files'), "data-post-client_id" => $client_id));
                    }
                    ?>
                </div>
            </div>
...
<script type="text/javascript">
    $(document).ready(function () {
        $("#client-file-table").appTable({
            source: '<?php echo_uri("check_in/files_list_data/" . $client_id) ?>',
            order: [[0, "desc"]],
            columns: [
