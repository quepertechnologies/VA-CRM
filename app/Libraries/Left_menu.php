...
class Left_menu
{
    ...
    private function _get_sidebar_menu_items($type = "")
    {
        ...
        if ($this->ci->login_user->user_type == "staff" && $type !== "client_default") {
            ...
            $clients_submenu = array();

            if ($this->ci->login_user->is_admin || $access_client) {
                $clients_submenu['students'] = array("name" => "students", "url" => "students", "class" => "briefcase");
                $clients_submenu['migration_clients'] = array("name" => "migration_clients", "url" => "migration_clients", "class" => "briefcase");
                $clients_submenu['organizations'] = array("name" => "organizations", "url" => "organizations", "class" => "briefcase");
            }

            if (count($clients_submenu)) {
            ...
            if ($this->ci->login_user->is_admin || $access_client) {
                $sidebar_menu["services"] = array("name" => "services", "url" => "courses", "class" => "book-open");
            }
